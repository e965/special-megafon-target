...
export const U = {
    ...
    qsf: (qS, from, options = []) => {
        ...
        return options.includes('a') ? from.querySelectorAll(qS) : from.querySelector(qS);
    },
    ...
    prepareText(text) {
        ...
        if (matches.fakeLinks) {
            matches.fakeLinks.forEach(fakeLink => {
                ...
                text = text.replace(regExps.fakeLinks, createElement('span', 'mgfn-trgt__link', { innerText: _link[1].replace(/]/g, '') }).outerHTML);
            });
        }
        ...
        if (matches.bold) {
            matches.bold.forEach(bold => {
                ...
                text = text.replace(regExps.bold, createElement('b', '', { innerText: _bold[1].replace(/]/g, '') }).outerHTML);
            });
        }
        ...
        if (matches.quote) {
            matches.quote.forEach(quote => {
                ...
                text = text.replace(regExps.quote, createElement('q', '', { innerText: _quote[1].replace(/]/g, '') }).outerHTML);
            });
        }
